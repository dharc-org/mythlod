...
var defaults = {
  ...
  listItemTemplate   : '<li><div class="facetitem provy" id="<%= id %>"><%= name %><span class=facetitemcount>(<%= count %>)</span></div></li>',
...
$('.facetitem').click(function(event){
  $(settings.resultSelector).html(settings.results).addClass('pippi')
});*/
...
function createFacetUI() {
  ...
  _.each(settings.facets, function(facettitle, facet) {
    ...
    var facetlistHtml = $(facetlisttemplate(facetItem));

    facetHtml.append(facetItemHtml);
    var facetlist = $(facetlistHtml); // prima filtrava un'altra cosa
    _.each(settings.facetStore[facet], function(filter, filtername){
      ...
      facetlist.append(filteritem); // e qui
    });
  ...
  order();   /* guarda se qui puoi cambiare l'ordine */
  updateFacetUI();  // questo lo hai rimosso ricordati
  updateResults();
  /* */
});
  ...
  $(settings.bottomSelector).append(bottom);
...
// this extra function is needet to add the perseus snippet into each item card 

function perseus_text(obj) {

var arr2 = []
  if (obj.perseus_links && obj.cits_labels) {
  item_refs_to_perseus = obj.perseus_links.split('@')
      for (pl in item_refs_to_perseus) {
        var arr1 = []
          for (var z in perseus_data) {
              if (item_refs_to_perseus[pl] === perseus_data[z]['urn_cts']) {
                urn_cts_id = 'cit' + pl + obj.item.replace('https://dharc-org.github.io/mythologiae/item/', '_item_') // on purpose id to trigger button, each id is univoque by using item_n + cit local index
                arr1.push(perseus_data[z]['urn_cts'], obj.cits_labels[pl], perseus_data[z]['corresp_string'], urn_cts_id)
                arr2.push(arr1)
      }}
      // where arr1[3] is the identifier for trigger button/collaps, arr1[0] is the perseus link, 
      // arr1[1] is citation label, arr1[2] is the text snippet took from perseus. 
